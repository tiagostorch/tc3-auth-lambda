...
output "mail_url" {
  description = "Rota interna para emisṣo de e-mails. Exige x-mail-api-token."
  value       = "${aws_apigatewayv2_api.http.api_endpoint}/mail"
}

output "ssm_mail_api_token_name" {
  description = "Par̢metro que deve ser sincronizado com MAIL_LAMBDA_TOKEN no Kubernetes."
  value       = aws_ssm_parameter.mail_api_token.name
}

output "ssm_mail_smtp_parameter_names" {
  description = "Par̢metros SecureString que precisam existir antes do deploy da Lambda de e-mail."
  value = {
